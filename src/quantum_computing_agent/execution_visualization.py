...
def _draw_circuit(ax, circuit, instructions) -> None:
    ...
    for column, instruction in enumerate(shown_ops, start=1):
        ...
        for y in ys:
            ...
            box = FancyBboxPatch((x - 0.28, y - 0.2), 0.56, 0.4, boxstyle="round,pad=0.03,rounding_size=0.04", facecolor=color, edgecolor="none", zorder=2)
            ax.add_patch(box)
            ax.text(x, y, label[:5], color="#161616", ha="center", va="center", fontsize=7, weight="bold")

def render_execution_dashboard(circuit, counts: dict[str, int], *, backend_name: str, job_id: str, shots: int) -> Path:
