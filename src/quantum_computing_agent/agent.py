...
Model: configurable via COMPUTING_MODEL (Granite/Ollama or Watsonx)
Port: 8003
...
# Instructions for the Computing Agent
...
def _extract_qasm(request: str) -> str | None:
    ...
    if not qasm_match:
        return None
    return re.split(r"\n\s*---\s*\n", qasm_match.group(0), maxsplit=1)[0].strip()
...
async def _create_execution_canvas(text: str, qasm_code: str) -> tuple[str, AgentArtifact | None]:
    ...
    try:
        uploaded = await File.create(filename=f"{job_id}_execution_results.png", content=dashboard_path.read_bytes(), content_type="image/png")
    ...
    if qasm_code:
        ...
        try:
            ...
            yield trajectory.trajectory_metadata(
                ...
                content="IBM Quantum returned a Job ID." if succeeded else "See the response for details.",
            )
            yield AgentMessage(text=response)
        except Exception as error:
            yield AgentMessage(text=f"❌ Error in Computing Agent: {explain_error(error)}")
        return
...
def run():
    ...
    print("=" * 80)
    print("🚀 Starting Quantum Computing Agent Server")
    print("=" * 80)
